...
const result1 = shouldFallbackOpenHouse(singleSlotMissingEndDate)
console.assert(result1 === true, '✅ Single-slot missing endDate triggers fallback')
...
const result2 = shouldFallbackOpenHouse(singleSlotWithEndDate)
console.assert(result2 === false, '✅ Single-slot with endDate does not trigger fallback')
...
const result3 = shouldFallbackOpenHouse(multiSlotValid)
console.assert(result3 === false, '✅ Multi-slot with valid occurrences does not trigger fallback')
...
const result4 = shouldFallbackOpenHouse(multiSlotMissingOccEndDate)
console.assert(result4 === true, '✅ Multi-slot with missing occurrence endDate triggers fallback')
...
const result5 = shouldFallbackOpenHouse(multiSlotMissingOccStartDate)
console.assert(result5 === true, '✅ Multi-slot with missing occurrence startDate triggers fallback')
...
const result6 = shouldFallbackOpenHouse(garageSaleNoEndDate)
console.assert(result6 === false, '✅ Non-open-house without endDate does not trigger fallback')

// Test 6a: Multi-slot open house with occurrences but missing root startDate should trigger fallback
console.log('\nTest 6a: Multi-slot open house with occurrences but missing root startDate should trigger fallback')
const multiSlotMissingRootStart = {
  '@context': 'https://schema.org',
  '@type': 'Event',
  'name': 'Open House - Weekend Showings',
  'description': 'Open house Saturday and Sunday',
  // Missing startDate at root level
  'endDate': '2026-01-04T13:00:00.000Z',
  'flypost': {
    'eventId': 'evt_test_006a',
    'category': 'open-houses',
    'realTimeData': true,
    'crawlable': true,
    'queryable': true
...
const result6a = shouldFallbackOpenHouse(multiSlotMissingRootStart)
console.assert(result6a === true, '✅ Multi-slot with missing root startDate triggers fallback')
...
console.log('  - Multi-slot open houses require root startDate and endDate ✅')
