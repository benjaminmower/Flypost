...
- `flypost.eventId`: Unique identifier (8-64 characters, alphanumeric + _ -)
  ...
  - **SERVER-GENERATED**: Always generated by server on insert, never from client input
  - **INSERT**: Server creates new eventId, ignoring any client-supplied value
  - **UPDATE**: Server preserves existing eventId when updating by eventIdentity
